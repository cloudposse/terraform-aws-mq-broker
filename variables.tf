...
variable "chamber_parameter_name" {
  type        = string
  default     = "/%s/%s"
  description = "Format to store parameters in SSM, for consumption with chamber (Requires `add_to_ssm=true`)"
}

variable "chamber_service" {
  type        = string
  default     = ""
  description = "SSM parameter service name for use with chamber. This is used in chamber_format where /$chamber_service/$parameter would be the default (Requires `add_to_ssm=true`)"
}

variable "overwrite_ssm_parameter" {
  type        = string
  default     = true
  description = "Whether to overwrite an existing SSM parameter (Requires `add_to_ssm=true`)"
}

variable "kms_key_id" {
  type        = string
  default     = ""
  description = "KMS key ID used to encrypt SSM parameters (Requires `add_to_ssm=true`)"
}
