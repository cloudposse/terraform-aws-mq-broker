...
locals {
  ...
  create_key              = var.kms_key_id == "" && var.add_to_ssm ? true : false
  key_id                  = length(var.kms_key_id) > 0 ? data.aws_kms_key.chamber_kms_key.0.id : var.add_to_ssm ? module.kms_key.0.key_id : ""
  chamber_service         = var.chamber_service == "" ? basename(pathexpand(path.module)) : var.chamber_service
  mq_admin_user           = length(var.mq_admin_user) > 0 ? var.mq_admin_user : random_string.mq_admin_user.0.result
  mq_admin_password       = length(var.mq_admin_password) > 0 ? var.mq_admin_password : random_string.mq_admin_password.0.result
  mq_application_user     = length(var.mq_application_user) > 0 ? var.mq_application_user : random_string.mq_application_user.0.result
  mq_application_password = length(var.mq_application_password) > 0 ? var.mq_application_password : random_string.mq_application_password.0.result
}
...
resource "aws_mq_broker" "default" {
  ...
  security_groups            = var.use_existing_security_groups == false ? [aws_security_group.default.0.id] : var.security_groups
...
resource "aws_security_group_rule" "default" {
  count                    = var.use_existing_security_groups == false && length(var.security_groups) > 0 ? length(var.security_groups) : 0
  type                     = "ingress"
  protocol                 = "tcp"
  from_port                = "0"
  to_port                  = "0"
